...
function addAutocomplete (inputElement, options) {
    ...
    if (typeof(options.source) == "undefined") {
		options.source = function (request, response) {autocompleteGeocoderAjax (options.sourceUrl, request, response);}
	}
    ...
    if (typeof(options.select) == "undefined") {
		...
		options.select = autocompleteCentreMap;
	}
...
function autocompleteCentreMap (result) {
	...
	if (typeof(map) != "undefined") {
		...
		// Zoom to at least 16
		var zoomTo = map.getZoom() < 16 ? 16 : map.getZoom();

		// Set the centre
		map.setView([result.lat, result.lon], zoomTo);
	}
}


// Function used to retrieve results from a Nominatim server
function autocompleteGeocoderAjax (url, request, response) {
	...
	if ($("#map")) {
		...
		urlParams.viewbox = autocompleteMapBoundsCallBack();
    };
    ...
    $.ajax({
		...
		success: function ( data, textStatus, jqXHR ) {
	    	response ( $.map( data, filterNominatimResult));
		},
...
// Equipped to handle CycleStreets standard uses of both OpenLayers and Leaflet
function autocompleteMapBoundsCallBack () {
...
function filterNominatimResult (item) {

    // Extract the street and neighbourhood
    var sn = streetNeighbourhood (item.display_name);
...
function streetNeighbourhood (display_name) {
