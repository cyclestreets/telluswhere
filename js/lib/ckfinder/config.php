...
if (isSet ($_SESSION['CKFinderAccessControl'])) {
	$config['AccessControl'] = $_SESSION['CKFinderAccessControl'];
} else {
...
if (isSet ($_GET['type']) && $_GET['type'] == 'Images') {
	$type = $_GET['type'];	// Cache to local variable
	$_GET['type'] = $_SERVER['SERVER_NAME'];	// Disable switching by type
}

$config['DefaultResourceTypes'] = $_SERVER['SERVER_NAME'];	// Use the site URL so this appears on the left tree menu as the root item

$config['ResourceType'][] = Array (
		'name' => $_SERVER['SERVER_NAME'],				// Single quotes not allowed
		'url' => $baseUrl,
		'directory' => $baseDir,
		'maxSize' => 0,
		'allowedExtensions' => ($type == 'Images' ? 'gif,jpeg,jpg,png' : /* 'htm,' . */ '7z,aiff,asf,avi,bmp,csv,doc,docx,fla,flv,gif,gz,gzip,jpeg,jpg,mid,mov,mp3,mp4,mpc,mpeg,mpg,ods,odt,pdf,png,ppt,pptx,pxd,qt,ram,rar,rm,rmi,rmvb,rtf,sdc,sitd,swf,sxc,sxw,tar,tgz,tif,tiff,txt,vsd,wav,wma,wmv,xls,xlsx,zip' . ',data,f90,html,xml'),
		'deniedExtensions' => '');
...
*/
#!# Set to false because isImageValid() fails for SWF files
...
*/
$config['HideFolders'] = Array(".*", "CVS",'.AppleDouble','sitetech','cgi-bin','_thumbs','_thumbnails');
...
###
### MUST ALSO ADD A PATCH AS FOLLOWS:
### $str = preg_replace ('/[^.-_a-z0-9]/', '', strtolower ($str));
### to the end of convertToAscii() in core/connector/php/php5/Utils/FileSystem.php
###

###
### MUST ALSO ADD A PATCH AS FOLLOWS: Delete the recursive unlinking routine:
### /*
###         $files = scandir($path);
###         if ($files) {
...
### in unlink() in core/connector/php/php5/Utils/FileSystem.php
###
