...
class news
{
	...
	public function main ()
	{
		...
		# Get articles
		$data = $this->databaseConnection->select ($this->settings['database'], 'news', $conditions, array (), $associative = false, $orderBy = 'id DESC', $limit = 10);
	...
	private function articleManipulate ($html, $article, $area)
	{
		...
		if ($article = $this->form ($html, $article, $area)) {
			...
			# Update the article
			if (!$this->databaseConnection->{$do} ($this->settings['database'], 'news', $article, $constraint)) {
	...
	private function articleDelete ($html, $article)
	{
		...
		# Do deletion
		if (!$this->databaseConnection->delete ($this->settings['database'], 'news', array ('id' => $article['id']))) {
